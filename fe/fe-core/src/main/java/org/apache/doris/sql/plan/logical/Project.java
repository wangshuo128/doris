...
public class Project extends LogicalPlan {
    public final List<NamedExpression> projectList;
    ...
    public List<Attribute> getOutput() {
        if (output == null) {
            output = projectList.stream()
                         .map(NamedExpression::toAttribute).collect(Collectors.toList());
        }
        return output;
    }
    ...
    public String toString() {
        return new StringBuilder("Project(")
                   .append(StringUtils.join(projectList, ", "))
                   .append(")").toString();
    }
