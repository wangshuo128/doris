...
class SelectRollupTest extends TestWithFeService implements PatternMatchSupported {
    ...
    public void testDup() throws Exception {
        ...
        Plan plan = PlanChecker.from(connectContext)
                .analyze("select k2, sum(v1) from dup_tbl group by k2")
                .getPlan();

        PlanChecker.from(connectContext)
                .withNereidsPlanner(planner -> {
                    planner.plan(LogicalPlanAdapter.of(plan));
                });
    }
    ...
    @Ignore
    void testTranslate() {
        Plan analyzed = PlanChecker.from(connectContext)
                .analyze("SELECT k2, sum(v1) FROM t2 GROUP BY k2")
                .getPlan();

        PlanChecker.from(connectContext)
                .withNereidsPlanner(planner -> {
                    planner.plan(LogicalPlanAdapter.of(analyzed));
                });
    }
    ...
    void testTranslate1() {
        Plan analyzed = PlanChecker.from(connectContext)
                .analyze("select k1, v1 from only_base group by k1;")
                .getPlan();
